      ...
      <section class="bg-white rounded-sm border border-slate-200/70 shadow-sm overflow-hidden">
        ...
        <div class="p-5 space-y-4">
          <div class="relative">
            ...
            <input [(ngModel)]="busqueda" placeholder="Buscar equipo por serial o nombre..." class="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-sm text-xs placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#044e46]" />
          </div>
          ...
          <button (click)="hoja()" [disabled]="!equipoId" class="w-full inline-flex items-center justify-center gap-2 bg-[#044e46] hover:bg-[#033b35] text-white text-xs font-semibold px-4 py-2.5 rounded-sm shadow-sm disabled:opacity-40 cursor-pointer">
      ...
      <section class="bg-white rounded-sm border border-slate-200/70 shadow-sm overflow-hidden">
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="grid gap-3">
            <button (click)="cert()" class="w-full inline-flex items-center justify-center gap-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-800 text-xs font-medium px-4 py-2.5 rounded-sm cursor-pointer">
            ...
            <div class="flex gap-2">
              <select [(ngModel)]="equipoId" class="flex-1 h-9 rounded-sm border border-slate-200 bg-white px-3 text-xs cursor-pointer">
                @for (e of equipos(); track e.id) { <option [value]="e.id">{{ e.serial }} — {{ e.nombre }}</option> }
              </select>
              <button (click)="certEquipo()" [disabled]="!equipoId" class="inline-flex items-center gap-2 bg-[#044e46] hover:bg-[#033b35] text-white text-xs font-semibold px-4 py-2 rounded-sm disabled:opacity-40 cursor-pointer">
